...
export function ChapterNavigation({ locale }: { locale: Locale }) {
  ...
  return (
    <nav className="chapter-nav" aria-label={navigation.index}>
      <button type="button" className="chapter-nav__toggle" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
      ...
      <div className={`chapter-nav__menu ${open ? "is-open" : ""}`}>
        ...
        {chapterIds.map((id, index) => (
          <a key={id} href={`#${id}`} onClick={() => setOpen(false)}>{navigation.chapters[index]}</a>
        ))}
      </div>
    </nav>
  );
